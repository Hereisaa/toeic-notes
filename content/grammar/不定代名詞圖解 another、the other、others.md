...
```mermaid
flowchart TD
    A["空格要填不定代名詞"] --> B{"空格後面<br>有沒有接名詞？"}
    B -- "有名詞（當形容詞用）" --> C{"名詞是單數還是複數？"}
    C -- "單數" --> D["another + 單數<br>the other + 單數"]
    C -- "複數／不可數" --> E["other + 複數<br>the other + 複數"]
    B -- "沒有名詞（單獨當代名詞）" --> F{"要代替的是<br>一個還是多個？"}
    F -- "一個" --> G{"是剩下的<br>唯一一個嗎？"}
    G -- "是（總數固定、講完了）" --> H["the other"]
    G -- "不是，只是再一個" --> I["another"]
    F -- "多個" --> J{"是剩下的<br>全部嗎？"}
    J -- "是（總數固定、講完了）" --> K["the others"]
    J -- "不是，只是其中一些" --> L["others／some others"]
```
